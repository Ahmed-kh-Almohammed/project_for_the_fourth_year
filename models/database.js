...
db.getChildren = (id1,id2) => {
    return new Promise((resolve, reject) => {
        pool.query('SELECT * FROM Users WHERE motherFk= ? and fatherFk =?',
         [id1,id2], (error, user) => {
            if (error) {
                return reject(error);
            }
...
db.getNNchildren = (id1,id2) => {
    return new Promise((resolve, reject) => {
        pool.query('SELECT * FROM relatives WHERE userOneFk = ? and userTwoFk =? and isUser =? ',
         [id1,id2,0], (error, user) => {
            if (error) {
                return reject(error);
            }
            return resolve(user);
